...
function LoginForm() {
  ...
  useEffect(() => {
    ...
    if (hash.includes('access_token=')) {
      const redirectNext = new URLSearchParams(globalThis.location.search).get('next') ?? '/'
      globalThis.location.replace(`/auth/callback?next=${encodeURIComponent(redirectNext)}${hash}`)
    }
  }, [])
  ...
  function signInWithLine() {
    setLoading('line')
    globalThis.location.href = `/api/auth/line?next=${encodeURIComponent(next)}`
  }
    ...
    <>
      ...
      <div className="space-y-3">
        <button
          ...
          className="flex w-full items-center justify-center gap-3 rounded-xl bg-[#06C755] px-4 py-3 text-sm font-semibold text-white transition hover:bg-[#05b04c] disabled:opacity-60"
        >
          {loading === 'line' ? (
